...
const AuditCycles = () => {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/audits', formData);
      setShowModal(false);
      fetchAudits();
    } catch (err) {
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 space-y-6">
        ...
        <div className="flex justify-between items-center bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 shadow-xl">
          <div>
            ...
            <p className="text-gray-400 mt-1">Manage and track compliance inventory audits</p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-gray-300">
              ...
              <tbody className="divide-y divide-white/10">
                {loading ? (
                  <tr><td colSpan="6" className="px-6 py-8 text-center text-gray-400">Loading audits...</td></tr>
                ) : audits.length === 0 ? (
                  <tr><td colSpan="6" className="px-6 py-8 text-center text-gray-400">No audit cycles found.</td></tr>
                ) : (
                  audits.map(audit => (
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md border border-white/20 overflow-hidden">
            <div className="p-6">
              <h3 className="text-xl font-bold text-white mb-4">Create Audit Cycle</h3>
              <form onSubmit={handleCreate} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Audit Name</label>
                  <input 
                    type="text"
                    required
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full px-4 py-3 bg-black/20 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all placeholder-gray-500"
                    placeholder="Details about this audit..."
                    rows="3"
                  ></textarea>
                </div>
